...
doc::Collection::Collection(Collection *parent) : BaseCollectionObject(), m_parent {(parent != nullptr) ? parent->shared_from_this() : nullptr} {}
...
doc::Collection *doc::Collection::FindChildCollection(const std::string_view &name)
{
	...
	auto it = std::find_if(m_children.begin(), m_children.end(), [&front](const PCollection &child) { return child->GetName() == front; });
...
std::string doc::Collection::GetWikiURL() const { return detail::get_wiki_url(GetFullName(), false); }
